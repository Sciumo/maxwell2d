...
rc_read(char *file_name, FILE *err_file)
{
  ...
  if (!file_name) {
    ...
    if (!file) {
      if (err_file) {
	fprintf(err_file, "Error openning %s\n", file_name);
      }
      return NULL;
    }
  }
...
/* Find the first config file on the command line, returning an index
   ...
   "-" could in principle be returned. If a single "-" is found
   (before a "--") then it is taken to mean that stdin should be read:
   in this situation an index to the "-" argument of the argv array is
   returned, and which when intercepted by rc_read() will be taken to
   mean stdin. */
int
...
int
rc_assign_real(rc_data *data, char *param, rc_real *value)
{
  int status;
...
int
rc_assign_real_vector(rc_data *data, char *param,
...
/* As rc_assign_real_vector() except that if no reals are found then
   ...
   returned. */
int
rc_assign_real_vector_default(rc_data *data, char *param,
...
int *
rc_get_int_vector(rc_data *data, char *param, int *length)
{
  int *out = NULL;
  data = __rc_find(data, param);
  *length = 0;
...
rc_real *
rc_get_real_vector(rc_data *data, char *param, int *length)
{
  rc_real *out = NULL;
  data = __rc_find(data, param);
  *length = 0;
...
rc_real **
rc_get_real_matrix(rc_data *data, char *param, int *m, int *n)
{
  int length, i;
  rc_real *val, **pval;
  data = __rc_find(data, param);

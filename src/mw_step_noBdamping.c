/* mw_step_noBdamping.c -- One timestep of E-M simulation but with no
...
mw_step(mwDomain *domain)
{
  ...
  if (domain->mode & MW_MODE_EXY) {
    ...
    for (j = 0; j < domain->ny-1; j++) {
      for (i = 0; i < domain->nx-1; i++) {
	domain->Ex[j][i] = domain->damping[j][i]*domain->Ex[j][i]
	  + domain->dt*domain->forcing[j][i]*domain->Ex_forcing
	  + domain->Eprefix[j][i]*(domain->Bz[j+1][i+1] - domain->Bz[j][i+1]);
	domain->Ey[j][i] = domain->damping[j][i]*domain->Ey[j][i]
	  + domain->dt*domain->forcing[j][i]*domain->Ey_forcing
  ...
  if (domain->mode & MW_MODE_EZ) {
	...
	domain->Ez[j][i] = domain->damping[j][i]*domain->Ez[j][i]
	  + domain->dt*domain->forcing[j][i]*domain->Ez_forcing
    ...
    for (j = 0; j < domain->ny-1; j++) {
      for (i = 0; i < domain->nx-1; i++) {
	domain->Bx[j][i] = /*domain->damping[j][i]*/domain->Bx[j][i]
	  - dt_dx*(domain->Ez[j+1][i+1] - domain->Ez[j][i+1]);
	domain->By[j][i] = /*domain->damping[j][i]*/domain->By[j][i]
  ...
  if (domain->mode & MW_MODE_EXY) {
    ...
    for (j = 1; j < domain->ny-1; j++) {
      for (i = 1; i < domain->nx-1; i++) {
	domain->Bz[j][i] = /*domain->damping[j][i]*/domain->Bz[j][i]
  ...
  if (domain->mode & MW_MODE_VACUUM) {
	...
	for (i = 0; i < domain->nx-1; i++) {
	  domain->Ex_vacuum[j][i]
	    = domain->damping[j][i]*domain->Ex_vacuum[j][i]
	    + domain->dt*domain->forcing[j][i]*domain->Ex_forcing
	    + Eprefix_vacuum
	    *(domain->Bz_vacuum[j+1][i+1] - domain->Bz_vacuum[j][i+1]);
	  domain->Ey_vacuum[j][i]
	    = domain->damping[j][i]*domain->Ey_vacuum[j][i]
	    + domain->dt*domain->forcing[j][i]*domain->Ey_forcing
	    + Eprefix_vacuum
	...
	for (i = 1; i < domain->nx-1; i++) {
	  domain->Ez_vacuum[j][i]
	    = domain->damping[j][i]*domain->Ez_vacuum[j][i]
	    + domain->dt*domain->forcing[j][i]*domain->Ez_forcing
	    + Eprefix_vacuum
	...
	for (i = 0; i < domain->nx-1; i++) {
	  domain->Bx_vacuum[j][i]
	    = /*domain->damping[j][i]*/domain->Bx_vacuum[j][i]
	    - dt_dx*(domain->Ez_vacuum[j+1][i+1] - domain->Ez_vacuum[j][i+1]);
	  domain->By_vacuum[j][i]
	    = /*domain->damping[j][i]*/domain->By_vacuum[j][i]
	...
	for (i = 1; i < domain->nx-1; i++) {
	  domain->Bz_vacuum[j][i]
	    = /*domain->damping[j][i]*/domain->Bz_vacuum[j][i]
